...
class UserDao_MockTest {
    ...
    void testSave_AddsUserToList() {
        User newUser = new User(3, "Ahmed", "ahmed@gmail.com", "pass123");
        dao.save(newUser);
        List<User> users = dao.findAll();
        assertEquals(3, users.size());
        assertTrue(users.stream().anyMatch(u -> u.getName().equals("Ahmed")));
    }
    ...
    void testSave_UserCanBeRetrievedById() {
        ...
        assertEquals("ahmed@gmail.com", retrieved.getEmail());
    }
    ...
    void testUpdate_DoesNotModifyData() {
        ...
        assertEquals("Kaoutar", original.getName());
        assertEquals("kaoutar@gmail.com", original.getEmail());
    }
